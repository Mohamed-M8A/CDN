...
const UIDManager = {
    generate() {
        const now = new Date();
        const datePart = now.getFullYear().toString() + (now.getMonth() + 1).toString().padStart(2, '0') + now.getDate().toString().padStart(2, '0') + now.getHours().toString().padStart(2, '0') + now.getMinutes().toString().padStart(2, '0') + now.getSeconds().toString().padStart(2, '0');
...
let toast = document.getElementById("country-toast") || Object.assign(document.createElement("div"), {id: "country-toast"});
...
function setActiveCountry(code, updateUrl = !0) {
    if (!options) return;
...
if (selected && dropdown && options) {
    ...
    options.addEventListener("click", (e) => {
        ...
        toast.textContent = "تم اختيار: " + li.textContent.trim();
...
function updateCartWidget() {
    ...
    if (cart.length > 0) { cartCountElement.classList.add("active"); } 
    else { cartCountElement.classList.remove("active"); }
}

updateCartWidget();
window.addEventListener("storage", function(event) { if (event.key === "cart") { updateCartWidget(); } });
window.addEventListener("cartUpdated", updateCartWidget);
const cartWidget = document.getElementById("cart-widget-header");
if (cartWidget) {
    cartWidget.addEventListener("click", function() { window.location.href = "/p/cart.html"; });
}
...
window.addEventListener("scroll", function() {
    if (!backTop) return;
    if (this.pageYOffset >= 1000) { backTop.classList.remove("d-none"); } 
    else { backTop.classList.add("d-none"); }
}, false);
